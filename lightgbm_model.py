...
data_vars_plot_log = ["perJet_EleEFrac", "perJet_MuonEFrac", "perJet_TDCavg_energyWeight"] # vars that are only significant on a log plot
...
y_var_names = ["perJet_MatchedLLP_TravelTime"]
...
x_data_arrays = mc_tree.arrays(x_var_names + ["Pass_LLPMatched", "perJet_MatchedLLP_DecayR", "perJet_MatchedLLP_DecayZ"], library="np") 
y_data_arrays = mc_tree.arrays(y_var_names, library="np")

data_mask = x_data_arrays["Pass_LLPMatched"] == 1
 
length = np.hypot(x_data_arrays['perJet_MatchedLLP_DecayR'], x_data_arrays['perJet_MatchedLLP_DecayZ'])

y_data_arrays["perJet_MatchedLLP_TravelTime"] = y_data_arrays["perJet_MatchedLLP_TravelTime"] - (length/c)

del x_data_arrays['perJet_MatchedLLP_DecayR']
del x_data_arrays['perJet_MatchedLLP_DecayZ']
...
Y["perJet_MatchedLLP_TravelTime"] = pd.DataFrame(y_data_arrays["perJet_MatchedLLP_TravelTime"][data_mask])
...
# [721]   valid_0's rmse: 2.93231
# MSE: 8.598437073756918
# RMSE: 2.932309170902161
# MAE: 1.5553425016461804
# R-squared: 0.6952318382551101
...
bst.save_model("lightgbm_LLP_travel_time_results/LLP_travel_time_predict.txt")


def make_calibration_plot(t_true, t_pred, # actual and model-predicted travel times [ns]
                          figname="lightgbm_LLP_travel_time_results/LLP_travel_time_residual_plots.png",                 
    ...
    plt.title("LLP travel time residual")
...
plt.ylabel("var name")
plt.tight_layout()
plt.savefig("lightgbm_LLP_travel_time_results/feature_importance")
plt.close()
...
plt.legend()
plt.tight_layout()
plt.savefig("lightgbm_LLP_travel_time_results/LLP_travel_time_RMSE_results")
plt.close()
